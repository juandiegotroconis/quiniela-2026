...
export default function ProfileScreen() {
  const { user, submitted, isUpdatable, userPicks, topScorer, savePredictions, submitPredictions, updateAvatarColor, logout } = useAuth();
  const { getMember } = useData();
  const navigate = useNavigate();
  const me = user ? getMember(user.id) : undefined;

  const [activeColor, setActiveColor] = useState(me?.avatarColor ?? AVATAR_COLORS[0]);
  ...
  const handleSave = async (picks: Record<number, UserPickEntry>, scorer: TopScorerSuggestion | null) => {
      ...
      <PageContainer>
        <div className="profile-hdr">
          ...
          <div className="profile-hdr__info">
            <div className="profile-hdr__name">{user?.name ?? 'Your Profile'}</div>
            <div className="profile-hdr__rank-row">
              {me && (
                <>
                  <span className="profile-hdr__rank-label">
                    Rank <span className="profile-hdr__rank-num">#{me.rank ?? '—'}</span>
                  </span>
                  <PositionChange current={me.rank} previous={me.prevRank ?? me.rank} />
