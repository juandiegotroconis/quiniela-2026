-- Adds "Greatest streak" / "Worst streak" leaderboard widgets.
--
-- A "hit" is any FINISHED match where the member's prediction is exact or
-- tendency-correct (the same definition as exact_count + correct_count); a
-- "miss" is a wrong prediction OR no prediction at all for a finished match.
-- Streaks are walked in chronological match order (matches.utc_date) across
-- ALL finished matches (not just the ones a member predicted), so streaks
-- are directly comparable between members regardless of how many picks each
-- one filled in.
--
-- best_streak  = longest consecutive run of hits
-- worst_streak = longest consecutive run of misses
...
returns table (user_id uuid, best_streak int, worst_streak int)
language sql
stable
...
as $$
  ...
  streaks as (
    select user_id, is_hit, count(*) as len
    from grouped
    group by user_id, is_hit, grp
  )
  select
    qm.user_id,
    coalesce(max(s.len) filter (where s.is_hit), 0) as best_streak,
    coalesce(max(s.len) filter (where not s.is_hit), 0) as worst_streak
  from quiniela_members qm
