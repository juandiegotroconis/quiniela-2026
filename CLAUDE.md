...
**Auth** (`src/lib/auth-context.tsx`): Supabase Auth, client-side only. `AuthProvider` listens to `onAuthStateChange`. On auth, loads quiniela membership (via `get_active_membership`, which returns `is_updatable`, `variant`, and `knockout_mode`), picks, top-scorer pick, bracket (matchup) picks, submission status, `isUpdatable`, and `avatarColor` from Supabase. `AuthUser` includes `avatarColor: string | null`. Exposes `user`, `loading`, `quinielaId`, `needsQuiniela`, `submitted`, `isUpdatable`, `knockoutMode`, `quinielaVariant`, `userPicks`, `bracketPicks`, `topScorer`, `login`, `signup`, `logout`, `joinWithCode`, `savePredictions`, `submitPredictions`, `updateAvatarColor`, `switchQuiniela`, `listUserQuinielas`, plus passkey helpers. `savePredictions(picks, scorer, bracketPicks)` now also upserts `bracket_predictions`.
...
- `showForm = isUpdatable || knockoutOpen` (where `knockoutOpen = isKnockoutEntryOpen(matches, knockoutMode, now)`). So the form also appears when the knockout window is open even if `isUpdatable` is false; otherwise `ProfileReadOnly` shows.
...
`refresh_quiniela_leaderboard(quiniela_id, match_id?)` (Postgres function) recomputes `quiniela_members` stats and rank via `ROW_NUMBER()` (tiebreak: `total_pts` → `exact_count` → `correct_count` → `joined_at`, so ranks are always unique, no ties), and inserts a `leaderboard_snapshots` row when `match_id` is given. Points come from `prediction_points_v2` (group + knockout/ET/penalties rules) plus `bracket_bonus_points` when a knockout prediction scored 0 — wired in by `20260628_fix_knockout_scoring.sql`.

`is_prediction_open(match_id, quiniela_id)` (Postgres function, `SECURITY INVOKER`) is the write-gate used by the `predictions`/`bracket_predictions` RLS policies: group → `quinielas.is_updatable`; knockout → R32 fully resolved, then `ONE_SHOT` open until the first R32 kickoff / `STAGE_BY_STAGE` open until the current stage's first kickoff (and only while it's the current stage). Mirrors the frontend gating in `helpers.ts`.
