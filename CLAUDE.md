...
8. `20260611_sync_live_matches_cron.sql` — (already applied via MCP) stores `project_url` and `anon_key` in Vault and registers two pg_cron jobs (`*/2 16-23 * * *` and `*/2 0-6 * * *`, UTC) that POST to the `sync-live-matches` Edge Function via `pg_net`. Requires the `pg_cron` and `pg_net` extensions (enabled).
...
The `sync-live-matches` Edge Function (`supabase/functions/sync-live-matches/index.ts`, deployed with `verify_jwt` on) runs every 2 minutes during the match window (16:00–06:58 UTC, via the two pg_cron jobs in migration 8 — kickoffs range 16:00–04:00 UTC). Each tick:
1. Queries `matches` for candidates: `utc_date` within the last 6 h (plus a 2-min lookahead) and `status != 'FINISHED'`. **If empty, exits without calling the API** — out-of-window ticks are free.
2. Makes **one** football-data.org request (`dateFrom`/`dateTo` = ±1 day; the API only filters by calendar date, not timestamp) covering all candidates.
3. Updates each candidate's `status`, scores (regular = `regularTime ?? fullTime`; ET columns = `fullTime` when `duration != 'REGULAR'`; penalties), `duration`, `winner`, `minute`, `last_synced_at`. Writes are skipped while the API still reports a non-live unchanged status.
4. Leaderboard refresh is **not** called here — the `trg_match_finished` trigger on `matches` (`on_match_finished()`) calls `refresh_quiniela_leaderboard(quiniela_id, match_id)` whenever `status` transitions to `FINISHED`.
